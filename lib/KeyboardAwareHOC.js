...
const _KAM_STATUSBAR_HEIGHT: number = 25
              ...
              UIManager.measureInWindow(
                ...
                (x: number, y: number, width: number, height: number) => {
                  ...
                  if (Platform.OS === 'ios') {
                    ...
                    const line = keyboardPosition - _KAM_STATUSBAR_HEIGHT - this.props.extraScrollHeight;

                    if (textInputBottomPosition >= line ) {
                      let overshotBy = textInputBottomPosition - line;

                      // Dirty hack. For some reason elements just around the edge of the keyboard
                      // fail to report their position properly. We overcompensate here.
                      if (overshotBy < this.props.extraScrollHeight) {
                        overshotBy = this.props.extraScrollHeight;
                      }

                      console.warn(textInputBottomPosition, line, overshotBy, (this.position.y + overshotBy + height));
                      this.scrollToPosition(
                        0,
                        // this.position.y + totalExtraHeight + (keyboardPosition - textInputBottomPosition),
                        this.position.y + overshotBy + height,
                        true
                      )
                    }
                  }
